...
### Fixed — compliance enforcement (correctness)

Compliance evaluation now fails closed across every SDK engine. Earlier versions could permit a transfer when compliance state was missing, stubbed, or unverifiable.

- All nine §4.4 rule categories evaluate fail-closed, in ascending priority with the first blocking rule terminating (§4.5, Invariant I₂). Missing inputs, unknown rule types, and unverifiable §10.15 oracle attestations now block.
- TypeScript and Java compliance engines rewritten to the fail-closed model (previously stubbed or partial). Python, Rust, and Go aligned to the Go reference behavior.
- Python no longer emits a synthetic state-gate rule for inactive assets, matching the reference (the state gate is a precondition, not a compliance rule).
- Solidity enforces identity validity, ACTIVE asset state, and a configured compliance oracle before any state mutation.

### Added

- `test-vectors/compliance_vectors.json` — 31-case cross-language compliance decision corpus; golden `(allowed, blockedBy)` expectations generated by the Go reference engine.
- Conformance harnesses in all five SDK languages (Go, TypeScript, Python, Rust, Java) that run the corpus against the shipped engine and assert identical decisions, wired into CI.

### Note

SDK reference-implementation patch only. The L3RS-1 standard version is unchanged (L3RS-1 v1.0.0).
